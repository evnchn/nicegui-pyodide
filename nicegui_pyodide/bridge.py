...
class _FakeEngineIO:
    """Satisfies outbox.py's access to ``core.sio.eio.ping_interval`` / ``ping_timeout``."""
    ping_interval = 25
    ping_timeout = 120


def _encode_bytes(obj: Any) -> Any:
    """Convert ``bytes`` values in a structure to ``{"__b64": "..."}`` for JSON serialization.
...
class PyodideBridge:
    """Replaces ``socketio.AsyncServer`` for in-browser Pyodide communication.

    All ``emit()`` calls are forwarded to JavaScript via
    ``window.niceguiBridge.handleMessage()``.
    """

    def __init__(self) -> None:
        self.eio = _FakeEngineIO()

    async def emit(self, event: str, data: Any, room: str | None = None, **kwargs: Any) -> None:  # pylint: disable=unused-argument
        """Emit an event to the JavaScript frontend.

        :param event: the event type (e.g. 'update', 'run_javascript', 'notify')
        :param data: the data payload
        :param room: the target client ID (ignored in Pyodide; there is only one client)
        """
        from js import window  # type: ignore  # pylint: disable=import-outside-toplevel,import-error

        from nicegui import json  # pylint: disable=import-outside-toplevel
        data_json = json.dumps(_encode_bytes(data))
        # handleMessage may return a JS Promise for async handlers (e.g. load_js_components).
